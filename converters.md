...
**Two-address `select`.** A `select` reads its live value at one address and
writes commands to another:
...
goes to the command address. Omit `state_address:` for a single-address select,
which reads and writes the one `address:`. This supports controls such as
party and economy mode, whose live state is read at a different register than
the command.
